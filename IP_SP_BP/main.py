...
if __name__ == "__main__":
    ...
    while(cap.isOpened()):
        ret, frame = cap.read()
        tmp = frame.copy()
        tmp = cv.cvtColor(tmp, cv.COLOR_BGR2YCR_CB)
        if frame is None:
            break
        ...
        if face.display == True:
            ...
            gray_Y = cv.bitwise_and(Y, Y, mask = cv.cvtColor(face.mask, cv.COLOR_BGR2GRAY))

            cv.imshow("Y channel", gray_Y)

            tmp = face.filterBySkinColor("Yellow", tmp)

            tmp = cv.bitwise_and(tmp, tmp, mask = cv.cvtColor(face.mask, cv.COLOR_BGR2GRAY))


        if type(points) == type(None):
            continue

        cv.imshow("Color convertion", tmp)
